...
$(document).ready(function(){
	...
	chessMech.live_chessboard_matrix = chessMech.chessTool.live_chessboard_matrix_gen(chess_place_ids, chess_piece_ids);
	var matrixIsSame = chessMech.chessTool.matrixSame(chessMech.live_chessboard_matrix, new_chessboard_matrix);
	var diffPiece = chessMech.chessTool.findDiffentPiece(chessMech.live_chessboard_matrix, new_chessboard_matrix);
...
class ChessMechanics{
	...
	getMovable(pieceId, x, y){
		...
		if(this.chessTool.isType(pieceId, "pawn")){
			...
			movablePlaces = this.rookExtraMoves(this.shrinkContinuosArray(this.getRookMovablePlaces(x, y)));
		}
	...
	getRookMovablePlaces(x, y){
		...
		if(x>=0&&x<=7&&y>=0&&y<=7){
			for(var i=7; i>=0; i--){
				if(i<x){
					var nextElement = document.getElementById(this.chessTool.id_gen(y, i));
					if(nextElement!=null){
			...
			for(var j=0; j<=7; j++){
				if(j>x){
					var nextElement = document.getElementById(this.chessTool.id_gen(y, j));
					if(nextElement!=null){
			...
			for(var k=0; k<=7; k++){
				if(k>y){
					var nextElement = document.getElementById(this.chessTool.id_gen(k, x));
					if(nextElement!=null){
			...
			for(var n=7; n>=0; n--){
				if(n<y){
					var nextElement = document.getElementById(this.chessTool.id_gen(n, x));
					if(nextElement!=null){
		...
		left =  this.chessTool.moveArrayToBack(left);
		right = this.chessTool.moveArrayToBack(right);
		up = this.chessTool.moveArrayToBack(up);
		down = this.chessTool.moveArrayToBack(down);
